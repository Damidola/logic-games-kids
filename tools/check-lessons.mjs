...
const moves = p => { const r = []; for (const [f, ds] of p.allDests()) for (const t of ds) r.push({ from: f, to: t, promotion: p.board.get(f).role === 'pawn' && (t >> 3 === 7 || t >> 3 === 0) ? 'queen' : undefined }); return r; };
...
for (const [k, l] of Object.entries(LESSONS)) for (const [i, it] of l.items.entries()) {
  try {
    ...
    else { const p = pos(it.task); const sols = moves(p).filter(m => judge(p, m, it.ok)).map(makeUci); if (!sols.length) throw new Error('no solution ' + k + '#' + i); console.log(k, i, 'task', sols.join(' ')); }
