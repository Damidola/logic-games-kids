class Game {
    ...
    makeMove(fromRow, fromCol, toRow, toCol) {
        ...
        if (!this.hasValidMovesForPlayer(opponentColor)) {
            this.gameOver = true;
            const again = { onAgain: () => this.startNewGame(), delay: 700 };
            if (this.currentPlayer === 'white') { // White made the move, Black has no moves -> White wins
                LG.win('Робот застряг — у нього не лишилося ходів!', again);
            } else { // Black made the move, White has no moves -> Black wins (Player loses)
                LG.lose('Твоя тура застрягла. Наступного разу залиш собі більше вільного місця!', again);
            }
    ...
    updateGameInfo() {
        document.getElementById('current-player').innerHTML = this.gameOver ? 'Гру завершено' :
            (this.currentPlayer === 'white' ? 'Твій хід <span class="mini white">♜</span>' : '🤖 Робот думає…');
