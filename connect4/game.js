document.addEventListener('DOMContentLoaded', () => {
    ...
    class ConnectFourGame {
        ...
        // Тема тепер одна — світла, зі спільного набору
        initializeTheme() {}

        // Підсвічуємо виграшну четвірку
        highlightWin() {
            ...
            for (let r = 0; r < this.ROWS; r++) {
                for (let c = 0; c < this.COLS; c++) {
                    ...
                    for (const [dr, dc] of dirs) {
                        ...
                        if (line.every(([rr, cc]) => rr >= 0 && rr < this.ROWS && cc >= 0 && cc < this.COLS && this.board[rr][cc] === p)) {
                            line.forEach(([rr, cc]) => {
                                const el = this.boardElement.querySelector(`.cell[data-row="${rr}"][data-col="${cc}"]`);
        ...
        // Повідомляємо про кінець гри через спільне вікно результату
        announceEnd() {
            this.highlightWin();
            const text = this.winnerMessage.textContent;
            const again = { onAgain: () => this.resetGame(), delay: 900 };
            if (text.startsWith('Ви')) LG.win('Ти зібрав чотири в ряд!', again);
            else if (text.startsWith('Нічия')) LG.draw('Усе поле заповнене, а четвірки немає.', again);
            else LG.lose('Робот зібрав четвірку. Подивись, де вона — наступного разу перекрий її раніше!', again);
        }
