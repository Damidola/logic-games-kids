...
const PieceFactory = {
    ...
    positionPiece(pieceElement, row, col) {
        // Позиція у відсотках, щоб поле масштабувалося під будь-який екран
        pieceElement.style.top = `${row * 12.5 + 1.25}%`;
        pieceElement.style.left = `${col * 12.5 + 1.25}%`;
    },
...
const GameController = {
    ...
    executeFishMove() {
        ...
        if (movableFish.length === 0) {
            ...
            LG.toast("Риби не можуть ходити — знову хід саламандри 🦎");
    ...
    endGame(message) {
        ...
        const again = { onAgain: () => this.reset(), delay: 500 };
        if (message.includes("Саламандра перемогла")) {
            LG.win('Саламандра 🦎 щасливо перебралася на інший берег!', again);
        } else {
            LG.lose('Риби 🐠 оточили саламандру. Спробуй обійти їх з іншого боку!', again);
        }
    },
